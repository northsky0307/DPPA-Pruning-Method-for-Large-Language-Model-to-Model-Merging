...
def get_results(pred_file, dev_set):
    ...
    def parse_pred_ans(pred_file,preds_str, golds_str, properties_list):
        num_q = 0
        acc = 0
        results = []
        preds = []
        ...
        with open(pred_file+'.txt', 'w') as file:
            file.write(str(float(acc / num_q)))
        acc_table = {}
    ...
    if dev_set in ['all', 'gsm8k', 'math', 'mathgpt', 'gsm8k_robust']:
        ...
        with open(f'./data/test/test.jsonl', 'r', encoding='utf-8') as f:
            for line in f:
                if dev_set != "all":
                    ...
                    properties.append({"source": json.loads(line)['source'], "tag": json.loads(line)['tag']})
        preds_str = []
        with open(pred_file+'.jsonl', 'r', encoding='utf-8') as f:
            for line in f:
                preds_str.append(json.loads(line)['response'])
        results, preds, golds = parse_pred_ans(pred_file, preds_str, golds_str, properties)
        with open(pred_file+'.jsonl', 'r', encoding='utf-8') as f:
        ...
        with open(pred_file+'.jsonl', 'w') as file:
            for item in data:
...
if __name__ == '__main__':
    ...
    if args.eval_only == False:
        ...
        another_args = {'max_num_batched_tokens': args.max_num_batched_tokens, 'max_model_len' : args.max_num_batched_tokens} 
        ...
        with open(args.output_file_name+'.jsonl', "w") as f:
